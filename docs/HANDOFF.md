...
## 2. Где остановились (3 августа 2026)

### Сделано в последних сессиях (скауты / админка)
...
### Продуктовое решение (согласовано, код ещё нет)

**Архив ЛС своих скаутов** (ops-бэкап), **не** релей-бот «все пишут боту».

- Пока сессия жива — sidecar копирует сообщения/edit/delete/(медиа) к нам.  
- Сгорел акк — в TG пусто, в Pulse архив остаётся.  
- MVP на старте: пользователь выбрал путь «архив»; следующий шаг имплементации — **сначала текст + edit/delete**, потом медиа (см. §7).
...
| identity card, prospects, logs | живые ЛС без архива |
...
### P0 — архив ЛС скаутов (решение принято)

1. Таблица `scout_message_archive` (account, peer, msg_id, text, direction, edited, deleted, ts).  
2. Sidecar: хук на new/edit/delete → POST в Java или писать в общую БД.  
3. Админка «Переписка»: fallback «из архива», если сессии нет.  
4. Потом: медиа → `scout_media/` + пути в БД; TTL качать сразу.

### P1 — скауты ops

- Суточный лимит join (например ≤60/акк) поверх 30с.  
- UI матрица «группы × скауты» с галками (модель уже memberships).  
- ≥2 живых PARSER/OBSERVER на один пул.  
- Разложить новые покупки (КЗ/UK/US/UZ) по ролям + прокси.  
- Карантин после FLOOD перед ACTIVE.

### P2 — продукт

- Outbound UX кампаний в боте.  
