...
public class AnalysisWorker {
    ...
    public void runAsync(Long requestId, long chatId) {
        ...
        try {
            ...
            log.info("Request {}: posts ready, total={}, category={}", requestId, sync.totalPosts(), sync.category());
            ...
            org.example.pulse_ai.stats.external.ExternalChannelMetrics external =
                    tgstatApiClient.getStat(channel.getUsername())
                            .orElseGet(() -> externalMetricsService.bestMetrics(channel.getUsername()));
            String externalSummary = externalMetricsService.describeForLlm(external);

            Double errPercent = external != null ? external.err() : null;
            ...
            Double reachPercent = subscribersForNiche > 0
            ...
            progress.step(3);
            String deepAnalysis = deepAnalysisService.analyzeChannel(
                    channel.getId(),
                    channel.getTitle(),
                    metrics,
                    request.getPeriodFrom(),
            ...
            boolean teaserMode = billingProperties.isEnabled() && request.getType() == RequestType.FREE;
            ...
            if (teaserMode) {
                resultHandler.deliverDeepAnalysisTeaser(chatId, requestId, deepAnalysis);
            } else {
